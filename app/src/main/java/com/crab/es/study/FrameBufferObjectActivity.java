...
import com.crab.es.study.fbo.FBORender;
...
public class FrameBufferObjectActivity extends AppCompatActivity implements FBORender.Callback {
    ...
    public void saveBitmap(final Bitmap b){
        ...
        if(!folder.exists()&&!folder.mkdirs()){
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(FrameBufferObjectActivity.this, "无法保存照片", Toast.LENGTH_SHORT).show();
                }
            });
            return;
        }
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(FrameBufferObjectActivity.this, "保存成功->"+jpegName, Toast.LENGTH_SHORT).show();
